...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['record_payment'])) {
    ...
    // Record transaction
    $stmt = $pdo->prepare("
        INSERT INTO fee_transactions (student_id, fee_allocation_id, amount, payment_date, payment_method, receipt_no, remarks, recorded_by) 
        VALUES (?, ?, ?, CURDATE(), ?, ?, ?, ?)
    ");
    ...
    // Record payment
    $stmt = $pdo->prepare("
        INSERT INTO fee_payments (allocation_id, amount_paid, payment_date, payment_method, receipt_no, remarks, received_by) 
        VALUES (?, ?, CURDATE(), ?, ?, ?, ?)
    ");
...
<body>
    ...
    <aside class="sidebar">
        <div class="sidebar-brand" style="padding: 20px;">
            <div class="sb-logo" style="color: white; font-size: 24px;">DSR</div>
            <span class="sb-sub" style="color: rgba(255,255,255,0.6);">Accountant Portal</span>
        </div>
        <nav style="padding: 16px;">
            ...
            <a href="fee_reports.php" style="display: block; padding: 10px; color: rgba(255,255,255,0.7); text-decoration: none; margin-top: 5px;">Reports</a>
        ...
        <div class="sidebar-profile" style="position: absolute; bottom: 0; padding: 20px;">
            ...
            <div class="profile-info">
                <span class="profile-name" style="color: white;"><?php echo htmlspecialchars($user_name); ?></span>
                <span class="profile-role" style="color: rgba(255,255,255,0.6);">Accountant</span>
            </div>
        </div>
    </aside>
    ...
    <main class="main-content">
        <div class="container">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-value" style="color: #ffc107;"><?php echo $pendingFees; ?></div>
                    <div>Pending Allocations</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" style="color: #dc3545;">Rs <?php echo number_format($pendingAmount); ?></div>
                    <div>Pending Amount</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" style="color: #28a745;">Rs <?php echo number_format($todayCollection); ?></div>
                    <div>Today's Collection</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" style="color: #17a2b8;">Rs <?php echo number_format($monthCollection); ?></div>
                    <div>This Month</div>
                </div>
            </div>
                ...
                <form method="POST">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="settings[late_fee_percentage]" value="<?php echo $settings['late_fee_percentage'] ?? '5'; ?>" step="0.01">
                        </div>
                        <div class="form-group">
                            <label>Default Discount (%)</label>
                            <input type="number" name="settings[discount_percentage]" value="<?php echo $settings['discount_percentage'] ?? '10'; ?>" step="0.01">
                        </div>
                        <div class="form-group">
                            <label>Tax Percentage (%)</label>
                            <input type="number" name="settings[tax_percentage]" value="<?php echo $settings['tax_percentage'] ?? '0'; ?>" step="0.01">
                        </div>
                        <div class="form-group">
                            <label>Default Due Days</label>
                            <input type="number" name="settings[default_due_days]" value="<?php echo $settings['default_due_days'] ?? '15'; ?>">
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                <div style="overflow-x: auto;">
                    <table>
                        <thead>
                            <tr><th>Roll No</th><th>Student Name</th><th>Class</th><th>Amount</th><th>Paid</th><th>Due</th><th>Due Date</th><th>Status</th><th>Action</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach($dueFees as $fee): 
                            ...
                            <tr>
                                ...
                                <td>Rs <?php echo number_format($fee['paid_amount']); ?></td>
                                <td style="color: #dc3545;">Rs <?php echo number_format($due); ?></td>
                                <td><?php echo date('d M Y', strtotime($fee['due_date'])); ?></td>
                                <td class="status-<?php echo strtolower($fee['status']); ?>"><?php echo $fee['status']; ?></td>
                                <td>
                                    <button class="btn btn-sm btn-success" onclick="openPaymentModal(<?php echo $fee['id']; ?>, '<?php echo $fee['name']; ?>', <?php echo $due; ?>)">Receive Payment</button>
                                    <button class="btn btn-sm btn-warning" onclick="openDiscountModal(<?php echo $fee['id']; ?>, '<?php echo $fee['name']; ?>', <?php echo $fee['amount']; ?>, <?php echo $fee['paid_amount']; ?>)">Apply Discount</button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <div class="card">
                ...
                <div style="overflow-x: auto;">
                    <table>
                        <thead>
                            <tr><th>Receipt No</th><th>Student</th><th>Roll No</th><th>Amount</th><th>Payment Method</th><th>Date</th><th>Collected By</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach($transactions as $t): ?>
                            <tr>
                                <td><?php echo $t['receipt_no']; ?></td>
                                <td><?php echo $t['student_name']; ?></td>
                                <td><?php echo $t['roll_no']; ?></td>
                                <td>Rs <?php echo number_format($t['amount']); ?></td>
                                <td><?php echo $t['payment_method']; ?></td>
                                <td><?php echo date('d M Y', strtotime($t['payment_date'])); ?></td>
                                <td><?php echo $t['collected_by']; ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
    ...
    <div id="paymentModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>Student</label>
                    <input type="text" id="payment_student_name" readonly>
                </div>
                <div class="form-group">
                    <label>Due Amount</label>
                    <input type="text" id="payment_due_amount" readonly>
                </div>
                <div class="form-group">
                    <label>Amount Receiving</label>
                    <input type="number" name="amount" step="0.01" id="payment_amount" required>
                </div>
                <div class="form-group">
    ...
    <div id="discountModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>Student</label>
                    <input type="text" id="discount_student_name" readonly>
                </div>
                <div class="form-group">
                    <label>Total Amount</label>
                    <input type="text" id="discount_total_amount" readonly>
                </div>
                <div class="form-group">
                    <label>Already Paid</label>
                    <input type="text" id="discount_paid_amount" readonly>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea name="discount_reason" rows="2" placeholder="Scholarship, Sibling discount, etc." required></textarea>
                </div>
                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button type="submit" class="btn btn-warning">Apply Discount</button>
                    <button type="button" class="btn btn-danger" onclick="closeDiscountModal()">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        window.onclick = function(event) {
            ...
            if (event.target == document.getElementById('discountModal')) closeDiscountModal();
        }
    </script>
</body>
</html>