...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_paid'])) {
    ...
    $stmt = $pdo->prepare("UPDATE fee_allocations SET status = 'Paid', payment_date = CURDATE() WHERE student_id = ? AND status != 'Paid'");
...
<body>
    ...
    <div class="container">
        ...
        <form method="POST" id="markPaidForm"><input type="hidden" name="student_id" id="paid_student_id"><input type="hidden" name="mark_paid" value="1"></form>
        
        <table>
            <thead><tr><th>Roll No</th><th>Student Name</th><th>Class</th><th>Total Fees</th><th>Paid</th><th>Due</th><th>Status</th><th>Action</th></tr></thead>
            <tbody id="feeTable">
                <?php foreach($students as $s): $status = $s['due'] == 0 ? 'Paid' : ($s['due'] > 0 ? 'Pending' : 'Pending'); $statusClass = strtolower($status); ?>
                <tr>
                    ...
                    <td><?php if($status != 'Paid' && $s['due'] > 0): ?><button class="pay-btn" onclick="markAsPaid(<?php echo $s['id']; ?>)">Mark Paid</button><?php else: ?>✓<?php endif; ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <script>
        function filterTable() { const input = document.getElementById('search'); const filter = input.value.toLowerCase(); const rows = document.getElementById('feeTable').getElementsByTagName('tr'); for (let i = 0; i < rows.length; i++) { const name = rows[i].getElementsByTagName('td')[1]?.innerText.toLowerCase() || ''; const roll = rows[i].getElementsByTagName('td')[0]?.innerText.toLowerCase() || ''; rows[i].style.display = (name.includes(filter) || roll.includes(filter)) ? '' : 'none'; } }
        function markAsPaid(studentId) { if (confirm('Mark all fees as paid for this student?')) { document.getElementById('paid_student_id').value = studentId; document.getElementById('markPaidForm').submit(); } }
